...
pathcell=[]
pathdict={}
tnode=int(input("Enter total nodes "))
if tnode<2:
    tnode=2
...
nline=tnode+1
start=(3300/((5*tnode)+9))
start=round(start)
#print(start)
startl=(start)/4
startl=round(startl)
#print(startl)
window=((tnode+2)*start)+((tnode+1)*startl) #complete window
if tnode>100:
    start=(3300//((5*tnode)+9))
    startl=start//4
    window=((tnode+2)*start)+((tnode+1)*startl)
#print(window)
...
pygame.init()
screen=pygame.display.set_mode((window,window),pygame.RESIZABLE)
